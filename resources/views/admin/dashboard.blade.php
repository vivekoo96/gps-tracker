<x-app-layout>
    ...
    <div class="mb-8">
        <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">GHMC Overview</h3>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm rounded-xl border border-gray-200 dark:border-gray-700">
                <div class="p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Total Zones</p>
                            <p class="text-2xl font-semibold text-gray-900 dark:text-gray-100">{{ $stats['total_zones'] }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm rounded-xl border border-gray-200 dark:border-gray-700">
                <div class="p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Total Circles</p>
                            <p class="text-2xl font-semibold text-gray-900 dark:text-gray-100">{{ $stats['total_circles'] }}</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-semibold text-gray-900 dark:text-gray-100">{{ $stats['total_wards'] }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm rounded-xl border border-gray-200 dark:border-gray-700">
                <div class="p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Transfer Stations</p>
                            <p class="text-2xl font-semibold text-gray-900 dark:text-gray-100">{{ $stats['total_transfer_stations'] }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm rounded-xl border border-gray-200 dark:border-gray-700">
                <div class="p-6">
                    <h4 class="text-base font-medium text-gray-900 dark:text-gray-100 mb-4">Vehicles by Type</h4>
                    <div class="space-y-4">
                        @foreach($stats['vehicles_by_type'] as $type => $count)
                            <div class="flex items-center justify-between">
                                <span class="text-sm text-gray-600 dark:text-gray-300 capitalize">{{ $type ?: 'Unspecified' }}</span>
                                <span class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ $count }}</span>
                            </div>
                            <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5">
                                <div class="bg-indigo-600 h-2.5 rounded-full" style="width: {{ $stats['total_devices'] > 0 ? ($count / $stats['total_devices']) * 100 : 0 }}%"></div>
                            </div>
                        @endforeach
                        @if($stats['vehicles_by_type']->isEmpty())
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm rounded-xl border border-gray-200 dark:border-gray-700">
                <div class="p-6">
                    <h4 class="text-base font-medium text-gray-900 dark:text-gray-100 mb-4">Vehicles by Zone</h4>
                    <div class="space-y-4 overflow-y-auto max-h-64">
                         @foreach($stats['zones_with_counts'] as $zone)
                            <div class="flex items-center justify-between">
                                <span class="text-sm text-gray-600 dark:text-gray-300">{{ $zone->name }}</span>
                                <span class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ $zone->devices_count }}</span>
                            </div>
                            <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5">
                                <div class="bg-green-500 h-2.5 rounded-full" style="width: {{ $stats['total_devices'] > 0 ? ($zone->devices_count / $stats['total_devices']) * 100 : 0 }}%"></div>
                            </div>
                        @endforeach
                    </div>
                </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- No Communication Widget -->
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm rounded-xl border border-gray-200 dark:border-gray-700">
                    <div class="p-6">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Communication Status</h3>
                            <span class="px-2 py-1 text-xs font-semibold bg-red-100 text-red-800 rounded-full">Alerts</span>
                        </div>
                        <div class="flex items-center">
                            ...
                            <div>
                                <div class="text-3xl font-bold text-gray-900 dark:text-gray-100">{{ $stats['no_communication_count'] }}</div>
                                <div class="text-sm text-gray-500">Vehicles Offline (>24h)</div>
                            </div>
                        </div>
                        <div class="mt-4">
                            <a href="{{ route('admin.tickets.create', ['alert_type' => 'NO_DATA']) }}" class="text-sm text-indigo-600 hover:text-indigo-900 font-medium">Raise Ticket &rarr;</a>
                        </div>
                    </div>
                </div>

                <!-- Speed Compliance Widget -->
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm rounded-xl border border-gray-200 dark:border-gray-700">
                    <div class="p-6">
                         <div class="flex items-center justify-between mb-4">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Speed Compliance</h3>
                             <span class="px-2 py-1 text-xs font-semibold bg-orange-100 text-orange-800 rounded-full">Today</span>
                        </div>
                        <div class="flex items-center">
                            <div class="p-3 bg-orange-50 dark:bg-orange-900/20 rounded-lg mr-4">
                             ...
                             <div>
                                <div class="text-3xl font-bold text-gray-900 dark:text-gray-100">{{ $stats['overspeed_count'] }}</div>
                                <div class="text-sm text-gray-500">Speed Violations</div>
                            </div>
                        </div>
                        <div class="mt-4">
                             <a href="{{ route('admin.tickets.index', ['alert_type' => 'SPEED']) }}" class="text-sm text-indigo-600 hover:text-indigo-900 font-medium">View Violations &rarr;</a>
                        </div>
                    </div>
                </div>
            </div>

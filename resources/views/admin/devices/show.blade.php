    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <div class="flex space-x-2">
                ...
                <a href="{{ route('admin.devices.index') }}" class="px-4 py-2 bg-gray-600 text-white rounded hover:bg-gray-700 flex items-center">
                    ← Back to Devices
                </a>
            </div>
        </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div class="bg-gray-800 rounded-lg p-4 flex items-center shadow-lg border border-gray-700">
                    ...
                    <div>
                        ...
                        <h3 class="text-white font-bold text-lg">{{ $device->name }}</h3>
                ...
                <div class="bg-gray-800 rounded-lg p-4 flex items-center shadow-lg border border-gray-700">
                 ...
                 <div class="bg-gray-800 rounded-lg p-4 flex items-center shadow-lg border border-gray-700">
                    ...
                    <div>
                        ...
                        <h3 class="text-white font-bold text-lg">0</h3>
                    </div>
                </div>
                ...
                <div class="bg-gray-800 rounded-lg p-4 flex items-center shadow-lg border border-gray-700">
                    ...
                    <div>
                        ...
                        <h3 class="text-white font-bold text-lg">0.00 km</h3>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gray-800 rounded-lg p-6 shadow-lg border border-gray-700">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="date" class="w-full bg-gray-700 border-gray-600 text-white rounded focus:ring-indigo-500 focus:border-indigo-500" value="{{ now()->subDays(7)->format('Y-m-d') }}">
                    </div>
                    <div>
                        <label class="block text-gray-400 text-xs mb-1">End Date</label>
                        <input type="date" class="w-full bg-gray-700 border-gray-600 text-white rounded focus:ring-indigo-500 focus:border-indigo-500" value="{{ now()->format('Y-m-d') }}">
                    </div>
                    <div class="flex items-end">
                        <button class="w-full bg-indigo-600 text-white px-4 py-2 rounded hover:bg-indigo-700 transition">Filter</button>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gray-800 rounded-lg shadow-lg border border-gray-700 overflow-hidden">
                <div class="border-b border-gray-700 px-6 pt-4 flex space-x-6">
                    <button @click="activeTab = 'trips'" :class="{'text-indigo-400 border-indigo-400': activeTab === 'trips', 'text-gray-400 border-transparent': activeTab !== 'trips'}" class="pb-3 border-b-2 font-medium transition">Trip History</button>
                    <button @click="activeTab = 'live'" :class="{'text-indigo-400 border-indigo-400': activeTab === 'live', 'text-gray-400 border-transparent': activeTab !== 'live'}" class="pb-3 border-b-2 font-medium transition">Live Data</button>
                    <button @click="activeTab = 'sensors'" :class="{'text-indigo-400 border-indigo-400': activeTab === 'sensors', 'text-gray-400 border-transparent': activeTab !== 'sensors'}" class="pb-3 border-b-2 font-medium transition">Sensors (Fuel/Cam)</button>
                </div>

                <div class="p-6">
                    <!-- TRIPS TAB -->
                    <div x-show="activeTab === 'trips'">
                        <h3 class="text-white font-bold mb-2">Trip History</h3>
                        <p class="text-gray-500 text-sm mb-6">Showing data from {{ now()->subDays(7)->format('Y-m-d') }} to {{ now()->format('Y-m-d') }}</p>
                    ...
                    <div x-show="activeTab === 'live'" style="display: none;">
                        ...
                        <div class="bg-black text-green-400 font-mono p-4 rounded h-64 overflow-y-auto text-sm border border-gray-700">
                             <p>[{{ now() }}] TCP Connected: {{ $device->unique_id }}</p>
                             <p>[{{ now() }}] AUTH: GT06 Protocol Verified</p>
                             <p>[{{ now() }}] HEARTBEAT: Status OK, IGN: Off</p>
                        </div>
                    </div>

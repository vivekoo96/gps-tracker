...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ isset($title) ? $title . ' - ' . site_setting('site_name', 'GPS Tracker') : site_setting('site_name', 'GPS Tracker') }}</title>
        
        <!-- Favicon -->
        <link rel="icon" type="image/x-icon" href="{{ site_setting('favicon', 'favicon.ico') }}">
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        @else
                            <div class="w-6 h-6 bg-gradient-to-br from-blue-600 to-blue-700 rounded-full flex items-center justify-center">
                                <span class="text-sm font-medium text-gray-600 dark:text-gray-300">{{ site_setting('site_name', 'Analogue IT') }}</span>
                            </div>
                        @endif
                    </div>
            ...
            <footer class="bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 mt-auto">
                <div class="max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-2">
                            @if(site_setting('logo'))
                                <img src="{{ site_setting('logo') }}" alt="Logo" class="h-5 w-auto">
                            @else
                            ...
                            <span class="text-sm text-gray-600 dark:text-gray-400">
                                © {{ date('Y') }} <span class="font-semibold">{{ site_setting('site_name', 'Analogue IT Solutions') }}</span>. {{ site_setting('footer_text', 'All rights reserved.') }}
                            </span>
                        </div>

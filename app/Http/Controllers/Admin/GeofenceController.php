...
class GeofenceController extends Controller
{
    ...
    public function update(Request $request, Geofence $geofence)
    {
        ...
        // Only update or create alert configuration if alert fields are provided
        if (isset($validated['alert_on_entry']) || isset($validated['alert_on_exit']) || isset($validated['notify_users'])) {
            $alert = GeofenceAlert::where('geofence_id', $geofence->id)->first();
            
            if ($alert) {
                // Update existing alert
                $alert->update([
                    'alert_on_entry' => $validated['alert_on_entry'] ?? $alert->alert_on_entry,
                    'alert_on_exit' => $validated['alert_on_exit'] ?? $alert->alert_on_exit,
                    'notify_users' => $validated['notify_users'] ?? $alert->notify_users,
                ]);
            } else {
                // Create new alert only if geofence has an ID
                if ($geofence->id) {
                    GeofenceAlert::create([
                        'geofence_id' => $geofence->id,
                        'alert_on_entry' => $validated['alert_on_entry'] ?? true,
